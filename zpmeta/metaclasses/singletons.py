...
class MultitonType(type):
    ...
    _instance_key = "params"  # the key used to identify the instance
    ...
    def __call__(cls, *args, **kwargs) -> object:
        ...
        if cls._instance_key is None:
            print("Warning: No instance key provided for class " + cls.__name__ + ". Using class name as key.")
            instance_key: str = cls.__name__
        else:
            print("Using instance key " + cls._instance_key + " for class " + cls.__name__ + ".")
            if len(args) > 0:
        ...
        try:
            obj = super(MultitonType, cls).__call__(*args, **kwds)
        except TypeError as err:
            if 'takes no arguments' in str(err):
                obj = super(MultitonType, cls).__call__()
            else:
                raise
